...
class LxmlParserLinkExtractor:
    def __init__(
        ...
        strip=True,
        canonicalized=False,
            extra_tags_xpath=None,
            extra_tags_json=None,
    ):
        if extra_tags_json is None:
            extra_tags_json = {}
        if extra_tags_xpath is None:
            extra_tags_xpath = {}
    ...
    def _extract_links(self, selector, response_url, response_encoding, base_url):
        ...
        for el, attr, attr_val in self._iter_links(selector.root):
            ...
            if self.extra_tags_xpath:
                for attr_name, extra_xpath in self.extra_tags_xpath.items():
                    for extra_tag in selector.xpath(extra_xpath):
                        if not extra_xpath:continue
                        # 提取自定义数据
                        custom_data = {
                            attr_name:extra_tag.extract_first()
                        }
                        # 检查当前链接元素是否与自定义数据相关联
...
class MyLxmlLinkExtractor:
    ...
    def _extract_links(self, *args, **kwargs):
        # todo
        # 传入 extra_tags_xpath ，key 和value ，除了解析url，也把自定义的东西解析出来
